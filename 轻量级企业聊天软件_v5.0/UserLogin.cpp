...
{
	ui.setupUi(this);
	setAttribute(Qt::WA_DeleteOnClose); //设置窗体属性,用于在关闭窗口时自动删除窗口部件，以确保资源得到适当的释放。
	initTitleBar();//初始化标题
...
bool UserLogin::veryfyAccountCode(bool &isAccountLogin, QString &strAccount)
{
	...
	if (queryEmployeeID.first()) //第一条结果有 结果,指向结果集第一条
	{
		...
		{
			gLoginEmployeeID = strAccountInput;//将输入的账号进行赋值到全局变量，方便后面使用
			isAccountLogin = false;//QQ号登录（非账号登录）
			strAccount = strAccountInput;//更改传入的参数strAccount：这是QQ号（比如10001）
			return true;
		}
		else
		{
...
{
	bool isAccountLogin;//是否为账号登录
	QString strAccount;//既可能是账号（名称），也可能是QQ号（员工号）
	if (!veryfyAccountCode(isAccountLogin, strAccount))//这两个值会改变，因为我们是引用传递参数(引用传参的用法)左值引用
	{
		QMessageBox::information(NULL, QString::fromLocal8Bit("提示"),
